use chess_engine_core::{
    BitBoard, CastleRightsType, Color, Piece, PieceMoves, PieceType, Rank, Square,
};
...
impl Board {
    ...
    pub fn generate_moves(&self) -> Vec<PieceMoves> {
            ...
            for square in self.piece_bitboard(Piece::new(piece_type, self.color())) {
                let to = match piece_type {
                    PieceType::Pawn => {
                        ...
                        let pawn_attacks =
                            get_pawn_attacks(color, square) & self.opponent_bitboard();
                ...
                if to != BitBoard::EMPTY {
                    pieces.push(PieceMoves {
                        piece: Piece::new(piece_type, self.color()),
                        from: square,
                        to,
                    });
                }
